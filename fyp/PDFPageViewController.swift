...
//    }
    ...
    if let drawObjects = self.pageDrawObjects[pageCurrent] {
      if drawObjects.count == 0 {
        ...
        let fileId = assignmentRecord!.refId
        self.api.getAnnotation(fileId: String(describing:fileId), pageId: String(pageCurrent+1)){
  ...
  func loadAssignmentRecordURL() {
    ...
    api.downloadAssignment(courseCode: course!.code, refId: (assignmentRecord?.refId)!){
        ...
        DispatchQueue.main.async(){
            ...
            for _ in 1...self.pageCount! {
                self.PDFViewControllers += [PDFViewController()]
            }
...
//    print("what is inside pageDrawObjects[currentPage]? \(self.pageDrawObjects[self.pageCurrent])")
  ...
  func doneBtnTapped(_ sender: UIButton){
    let alertController = UIAlertController(title: "Done", message: "Do you finish marking?", preferredStyle: UIAlertControllerStyle.alert)
    
    alertController.preferredContentSize = CGSize(width:400,height:500)
    
    alertController.addAction(UIAlertAction(title: "Yes", style: UIAlertActionStyle.default, handler: { [weak self] (_) in
        let innerAlert = UIAlertController(title: "Input the marks", message:nil, preferredStyle: .alert)
        innerAlert.addTextField { (textField: UITextField) in
        ...
        innerAlert.addAction(UIAlertAction(title: "OK", style: .default, handler:
            {action in
                ...
                let textField = innerAlert.textFields![0]
...
//  }
    ...
    func addMarking(refId: Int, courseCode: String, asgnNum: Int, status: Int, score: Int? = nil){
        print("addMarking# start")
        let connectorType = ConnectorType.Veriguide
        print("addMarking# connectorType=\(connectorType)")
        let api = AppAPI(connectorType: connectorType)
        if(api == nil){
            print ("Fail to load api")
        }
        api!.addAssignmentMarking(refId: refId, courseCode: courseCode, asgnNum: asgnNum, status: status, score: score){
            (result, error) in
            if (error != nil){
                //handle error here
                return
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    ...
    if segue.identifier == "backToAssignmentRecord" {
        ...
        assignmentRecordViewController.assignment = self.assignment
