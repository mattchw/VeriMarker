...
//  var scheduler = self.parentController!.scheduler
  ...
  func addAnnotation() {
    ...
    let fileId = (parentController!.assignmentRecord?.refId)!
    parentController!.scheduler.addAnnotation(fileId: String(describing:fileId), lastModifiedTime: Date(), pageDrawObjects: parentController!.pageDrawObjects)
